    ...
    return (
      <section className="container mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[1, 2, 3].map((index) => (
            <div key={index} className="animate-pulse bg-white/90 dark:bg-white/10 backdrop-blur-lg shadow-xl rounded-xl overflow-hidden border border-gray-200/20 dark:border-white/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {articles.map((article) => (
          <Link href={`/blog/${article.id}`} key={article.id} className="block">
            <div className="bg-white/90 dark:bg-white/10 backdrop-blur-lg shadow-xl rounded-xl overflow-hidden border border-gray-200/20 dark:border-white/20 transform hover:scale-105 transition-all duration-300">
              ...
              <div className="p-6">
                ...
                <p className="text-gray-800 dark:text-white/80 line-clamp-2">{article.content ? article.content.slice(0, 150).replace(/<[^>]+>/g, '') : ''}</p>
              </div>
            </div>
          </Link>
        ))}
